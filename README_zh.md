...
两层，因为只给结论不够，而不给结论也不行。

**`strengthFactors` —— 零权重要素账本。** 月令关系与旺相休囚死、逐支通根及其气位与
禄/刃/长生/墓库根标签、天干帮扶方向、帮抑计数。纯查表：无权重、无阈值、无结论词。
它是观察；想按自己流派的权重称量的调用方，可以直接从这里算。

**`strengthAssessment` —— 具名方法的参考判定。** 得分、判定、临界旗（离阈值 0.5 以内
时 `margin: '临界'`）、中和盘偏哪一侧。

定性规则与全部 20 个校准命例出自韦千里《千里命稿·评断篇》（1935，公版）。**数字不是。**
不存在可查证的通行权重表——问真八字、文墨天机闭源，网传口径互相矛盾。本项目的每一个
权重都是拟合的，并在 `method` 字段里如实标注。校准在 `scripts/strength-calibration/`：
`python3 calibrate.py` 打印 `172/10368`，9 个留出命例是**锁定参数后**打分、从未回调。
其中一例（阮玲玉）失败，原因可追溯到原书自己的话——「印绶冲散」，而 v1 未建模「冲」。
它被记录，不是被藏起来。

出生时辰未知时**整体缺席**：旺衰得分比它所依据的四柱对时辰更敏感。

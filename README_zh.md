...
每一张有流派分歧的表都是**具名输入**，`strengthFactors.conventions` 回写实际生效的一派、
未采用的各派、以及换派会改变哪些字段：

| 参数 | 默认 | 备选 |
|---|---|---|
| `twelveStageSchool` | 阳顺阴逆（渊海子平） | `yin_follows_yang` —— 阴阳同生同死（滴天髓） |
| `monthOrderSchool` | 以月支本气为令 | `earth_rules_final_18_days` —— 土旺四季十八日 |
| `bladeSchool` | 仅阳刃 | `yin_at_diwang`、`yin_at_guandai` |

`monthOrderSchool: "earth_rules_final_18_days"` 把四立前十八日判给土，之前仍归将尽之季
（辰木 未火 戌金 丑水）。**只有辰戌丑未月可能变**，其余八个月本气即当季五行。规则所依据的
距节天数随结果一起给出（`monthOrder.daysToNextJie`），解析出的令也给出
（`monthOrder.rulingElement`）——否则这个结论无法从输出独立复现。

**阴刃不给默认值。** 三派并存：帝旺说（乙寅 丁巳 己巳 辛申 癸亥）、冠带/禄前一位说
（乙辰 丁未 己未 辛戌 癸丑）、以及阴干根本不立刃。本包没有依据在其间取舍，
所以要么调用方具名指定，要么只拿到阳刃。阴日主尤其明显：十二长生这一岔路会改变每一个支，
辛 在 巳 按默认的《渊海子平》口径是 `死`，按《滴天髓》阴阳同生同死则是 `长生`。
传 `twelveStageSchool: "yin_follows_yang"` 即可切到后者，实际生效的流派回写在
`conventions.twelveStage`。阳日主在两派下完全一致。禄、刃取自《十干禄》与阳刃表，
**不随**这一岔路变动——只有长生会变。
